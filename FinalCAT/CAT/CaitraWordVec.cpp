...
CaitraWordVec::CaitraWordVec(string method)
{
    // readStopWords();
    m_method = method;
    threshold = 0;
...
CaitraWordVec::CaitraWordVec(string method, double errw, double stpError)
{
    // readStopWords();
    errorWeigth = errw;
    m_method = method;
    ...
    approximate_word_match_threshold = 1.0;
    match_last_word_window = 0;
    stopWordsError = stpError;
}

void CaitraWordVec::readWordVec(string fileName){
    wordVector.readWordVec(fileName);
}

void  CaitraWordVec::createGraph(vector<SearchGraphNode> v) {
    ...
    approximate_word_match.clear();
    lowercase_word_match.clear();
    suffix_insensitive_word_match.clear();
   ...
   // if(m_method == "WordVector" || m_method == "both")
    //    wordVector.cosineDistance.clear();
...
string CaitraWordVec::getPrefix(string pr){
    ...
    if(m_method =="WordVector"|| m_method == "both"){
        ...
        for(int p=0; p<prefix.size(); p++) {
            ...
            for (map<string, Word>::iterator iter = lexicon.begin(); iter != lexicon.end(); iter++) {
                ...
                double dis = wordVector.getDistance(idxInWordVec[prefix[p]], idxInWordVec[iter->second]);
                v.push_back(dis);
            }
            wordsSimilarity.push_back(v);
        }
    }
    ...
    float errorAllowed = prefix_matching_search( max_time, 0 );
...
// main search loop
float CaitraWordVec::prefix_matching_search( float max_time, float threshold ) {
    double start_time = get_wall_time();
    ...
    for( int state = 0; state < states.size(); state++ ) {
        ...
        for ( backIter back = states[state].back.begin(); back != states[state].back.end(); back++ ) {
            ...
            if (states[state].forward_score + back->score - errorWeigth * back->error < best_Score+threshold) {
                continue;
            }
            back_count++;
             ...
             */
                ...
                for ( matchIter match = matches.begin(); match != matches.end(); match++ ) {
                    ...
                    float er = process_match( state, *back, *match, *transition );
...
int CaitraWordVec::letter_string_edit_distance( Word wordId1, Word wordId2 ) {
    wstring word1;
...
inline vector< Match > CaitraWordVec::string_edit_distance( int alreadyMatched, const vector< Word > &transition ) {
    vector< Match > matches;
    int toMatch = prefix.size() - alreadyMatched;
    float **cost = (float**) calloc( sizeof( float* ), toMatch+1 );

    /*
    vector<string> stemsp, stemst;
    for(int i = 1; i <= toMatch; i++){
        string s1 = lowercase(surface[prefix[ alreadyMatched + i-1 ]]);
        Porter2Stemmer::stem(s1);
        stemsp.push_back(s1);
    }
    for(int j = 1; j <= transition.size(); j++){
        string s2 = lowercase(surface[transition[ j-1 ]]);
        Porter2Stemmer::stem(s2);
        stemst.push_back(s2);
    }
    ...
    */
    ...
    for( int i=0; i<=toMatch; i++ ) {
        ...
        cost[i] = (float*) calloc( sizeof(float), transition.size()+1 );
        for( int j=0; j<=transition.size(); j++ ) {
            if (i==0 && j==0) { // origin
                ...
                float lowestError = error_unit * (prefix.size() + transition.size());

                if (i>0) { // deletion

                    lowestError = cost[i-1][j] + error_unit;
                    if((m_method == "StopWords" || m_method == "both") && isInStopWords[prefix[ alreadyMatched + i-1 ]])
                        lowestError = cost[i - 1][j] + stopWordsError;
                }
                if (j>0) { // insertion
                    float thisError = cost[i][j-1] + error_unit;
                    if((m_method == "StopWords" || m_method == "both") && isInStopWords[transition[ j-1 ]])
                        thisError = cost[i][j - 1] + stopWordsError;
...
//                            float dist = 0;
                           // if(m_method == "WordVector" || m_method == "both")
                            //    dist = wordVector.getDistance(surface[transition[ j-1 ]], surface[prefix[ alreadyMatched + i-1 ]]);

                            if(last_word_may_match_partially &&
                                    ((alreadyMatched + i-1) == (prefix.size()-1)) && !prefix_has_final_space)
                                thisError += 1;
                            else{
                            ...
                            if(dist < 0.5)
                                ...
                                thisError += (1 - dist + 0.5);
             */


                                /*
                                if(stemsp[i - 1] != stemst[j - 1])
                                    thisError += 1;
 */                     //      }
                                //thisError += (1 - dist);
                                ...
                                if(m_method == "StopWords"){
                                    if(isInStopWords[transition[ j-1 ]] && isInStopWords[prefix[ alreadyMatched + i-1 ]]){
                                        
                                        thisError += stopWordsError;
                                    }else
                                        thisError+= 1;
                                }else if(m_method == "WordVector"){
                                    double sim = wordsSimilarity[alreadyMatched + i - 1][transition[j - 1]];
                                    if(sim < 0.4)
                                        thisError += 1;
...
inline float CaitraWordVec::process_match( int state, const BackTransition &back, const Match &match, const Transition &transition ) {
    int transition_to_state = transition.to_state;
    float score = back.score + transition.score;
    float error = back.error + match.error;
    ...
    // special case: all of the prefix is consumed
        ...
        /* if ( best.from_state == -1 || best.error > error || (best.error == error && (score > best.score ||
             (score == best.score && match.prefixMatched > best.prefix_matched) ))) {
             */
        if(best.from_state == -1 || (best.score - errorWeigth * best.error < score - errorWeigth * error) ||
                ((best.score - errorWeigth * best.error) == (score - errorWeigth * error) && match.prefixMatched > best.prefix_matched)){

            best_Score = max(best_Score, score - errorWeigth * error);
            best.score = score;
            best.from_state = state;
            best.transition = &transition;
            best.output_matched = match.transitionMatched;
            best.back_matched = back.prefix_matched;
            best.prefix_matched = match.prefixMatched;
            best.error = error;
            //cerr << "\t\t\tnew best\n";
...
string CaitraWordVec::lowercase(string mixedcase) {
    transform(mixedcase.begin(), mixedcase.end(), mixedcase.begin(), ::tolower);
    return mixedcase;
}

bool CaitraWordVec::equal_case_insensitive(string word1, string word2) {
