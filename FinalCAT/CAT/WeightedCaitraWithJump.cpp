...
namespace Caitra{
WeightedCaitraWithJump::WeightedCaitraWithJump()
{
    errorWeigth = -0.84;
    threshold = 0;
    max_time = 0;
...
WeightedCaitraWithJump::WeightedCaitraWithJump(double erw, const LanguageModelImplementation *lm,int jumpTH)
{
    errorWeigth = erw;
    threshold = 0;
    max_time = 0;
    ...
    match_last_word_window = 0;
    LM = lm;
    jumpLenThreshold = jumpTH;
}

void  WeightedCaitraWithJump::createGraph(vector<SearchGraphNode> v) {
...
string WeightedCaitraWithJump::getPrefix(string pr){
...
// main search loop
int WeightedCaitraWithJump::prefix_matching_search( float max_time, float threshold ) {
    double start_time = get_wall_time();
    ...
    for( int state = 0; state < states.size(); state++ ) {
        ...
        for ( backIter back = states[state].back.begin(); back != states[state].back.end(); back++ ) {
             ...
             */
                ...
                if(back->jumpState == 0 || (back->jumpState == 1 && back->numOfJumps < jumpLenThreshold)){ // jump this transition
...
int WeightedCaitraWithJump::letter_string_edit_distance( Word wordId1, Word wordId2 ) {
    wstring word1;
...
inline vector< Match > WeightedCaitraWithJump::string_edit_distance( int alreadyMatched, const vector< Word > &transition ) {
...
inline void WeightedCaitraWithJump::process_jump(int state, const BackTransition &back, const Transition &transition){
    ...
    for( backIter oldBack = states[transition_to_state].back.begin(); oldBack != states[transition_to_state].back.end(); oldBack++ ) {
        if (oldBack->prefix_matched == back.prefix_matched && oldBack->jumpState == 1) { // already a back path with same prefix match?
            ...
            if ((oldBack->score + errorWeigth * getLogErrorCost(oldBack->error) < score + errorWeigth * getLogErrorCost(error)) ||
                    ((oldBack->score + errorWeigth * getLogErrorCost(oldBack->error)) == (score + errorWeigth * getLogErrorCost(error)) && oldBack->back_matched < back.prefix_matched)){
...
inline int WeightedCaitraWithJump::process_match( int state, const BackTransition &back, const Match &match, const Transition &transition ) {
    int transition_to_state = transition.to_state;
    float score = back.score + transition.score;
...
inline double WeightedCaitraWithJump::getLMScore(string s){
...
string WeightedCaitraWithJump::lowercase(string mixedcase) {
    transform(mixedcase.begin(), mixedcase.end(), mixedcase.begin(), ::tolower);
    return mixedcase;
}

bool WeightedCaitraWithJump::equal_case_insensitive(string word1, string word2) {
