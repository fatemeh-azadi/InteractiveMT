...
void  ModifiedCaitra::createGraph(vector<SearchGraphNode> v) {
...
string ModifiedCaitra::getPrefix(string pr){
...
// main search loop
int ModifiedCaitra::prefix_matching_search( float max_time, float threshold ) {
    double start_time = get_wall_time();
...
int ModifiedCaitra::letter_string_edit_distance( Word wordId1, Word wordId2 ) {
    wstring word1;
...
inline vector< Match > ModifiedCaitra::string_edit_distance( int alreadyMatched, const vector< Word > &transition ) {
...
inline void ModifiedCaitra::process_match( int state, const BackTransition &back, const Match &match, const Transition &transition ) {
...
string ModifiedCaitra::lowercase(string mixedcase) {
    transform(mixedcase.begin(), mixedcase.end(), mixedcase.begin(), ::tolower);
    return mixedcase;
}

bool ModifiedCaitra::equal_case_insensitive(string word1, string word2) {
