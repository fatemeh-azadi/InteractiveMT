#ifndef WEIGHTEDCAITRAWITHJUMP_H
#define WEIGHTEDCAITRAWITHJUMP_H
...
class WeightedCaitraWithJump: public SearchGraph
{
    float threshold;
    float max_time;
    ...
    int transitionsSize;
    int jumpLenThreshold;
    const LanguageModelImplementation* LM;


public:
    WeightedCaitraWithJump();
    int getGraphSize() { return states.size(); }
    int getTransitionsSize(){ return transitionsSize; }

    WeightedCaitraWithJump(double erw, const LanguageModelImplementation* lm, int jumpTH);
    ~WeightedCaitraWithJump(){}
    ...
    inline void process_jump(int state, const BackTransition &back, const Transition &transition);
    inline double getLMScore(string s);
...
#endif // WEIGHTEDCAITRAWITHJUMP_H
